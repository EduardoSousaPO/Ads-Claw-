# Agent Loop (Reasoning Engine) Specification
...
4. Uma skill retorna um tool call exigindo usar "criar_arquivo".
...
| EC-02: Ferramenta retorna Throw (Error hard) | Tentou criar num path que não existe na máquina host do Node. | O catch manda: `{"error": "ENOENT path not exists..."}` como string de observação devolta pra IA corrigir caminho. |
...
- As Tools são injetadas em prompt - nunca expõe secrets nem paths internos completos que dão base para jailbreak explícito sem sanilização.
