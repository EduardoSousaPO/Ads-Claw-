...
- **Desvantagem:** Escalabilidade vertical limitada ao hardware do host local.
...
|------------|------------|-------------------------|
| **Linguagem** | **Node.js (TypeScript)** | Ambiente rápido para IO, ecossistema rico e familiaridade. |
| **Paradigma** | **Orientação a Objetos** | Uso obrigatório de Classes, Interfaces e Padrões de Projeto. |
...
1. **Facade:** `AgentController` e `MemoryManager` para simplificar interface com subsistemas.
2. **Factory:** `ProviderFactory` para instanciar diferentes provedores de LLM e `ToolFactory` para as ferramentas.
3. **Repository:** Para abstrair o acesso ao banco de dados SQLite.
...
|-------|---------|-----------|
| Corrupção do SQLite | Alto | Uso de WAL e backups locais periódicos. |
